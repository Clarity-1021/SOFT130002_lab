...
function showWindowHref(){
    let nameStart = url.value.search(/[?&]name=/);
    if(nameStart === -1){
        url_result.value = "没有名为name的参数";
        return ;
    }
    nameStart += 6;
    if(nameStart === url.value.length){
        url_result.value = "没有名为name的参数";
        return ;
    }
    url_result.value = url.value.substring(nameStart);
    let nameEnd = url_result.value.search(/&/);
    if(nameEnd === -1)
        return ;
    url_result.value = url_result.value.substring(0, nameEnd);
}
...
function arrSameStr(){
    let times = {};
    for(let i = 0, length = most.value.length; i < length; i++){
